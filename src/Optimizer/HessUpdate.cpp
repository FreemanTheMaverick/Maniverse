...
void BroydenFletcherGoldfarbShanno(Manifold& M1, Manifold& M2, EigenMatrix step1){
	const EigenMatrix S = M1.TransportManifold(step1, M2);
	const EigenMatrix Y = M2.Gr - M1.TransportManifold(M1.Gr, M2);
	const EigenMatrix YoverYS = Y / M2.Inner(Y, S);
	const EigenMatrix HS = M1.TransportManifold(M1.Hr(step1), M2);
	const EigenMatrix HSoverSHS = HS / M2.Inner(S, HS);
	if ( M1.MatrixFree ){
		M2.Hr = [&M1, &M2, S, Y, YoverYS, HSoverSHS](EigenMatrix v){
			const EigenMatrix Hv1 = M1.TransportManifold(M1.Hr(M2.TransportManifold(v, M1)), M2);
		...
		for ( int i = 0; i < M1.getDimension(); i++ ){
			const EigenMatrix new_vec = M1.TransportManifold(std::get<1>(M1.Hrm[i]), M2).reshaped<Eigen::RowMajor>();
